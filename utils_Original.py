...
### Parameters ###
aliases = {"Vanadium_Battery": "vrb", "Lithium_Battery": "li", "Super_Capacitor": "sc"}

default_config = {'Name': 'Default configuration for sizing tool',
                  'Comments': 'Default  config for sizing tool. Will be updated by user config. Do not alter values.',
                  'Timeseries': {'Type': 'Frequency',
                                 'Input_File': None,
                                 'Scaling': 1,
                  ...
                  'Controller': {'Class': 'controllers.SizingToolController',
                                 'Parameters': None},
                  'Sizing_Params': {'LifetimeSplits': 3,
                                    'Num_price_days':12,
                                    'Num_freq_days': 1,
                                    'UseParallel': True,
                                    'Max_iterations': 1000,
                                    ...
                                    'Tolerance': 0.001,
                                    'Delta_x': 0.01,
                                    'Parameterization_granularity': 8,
                                    'Price_day_weighting': [0.04657534246575343,
                                                            0.03287671232876712,
                                                            0.2328767123287671,
                                                            0.16712328767123288,
                                                            ...
                                                            0.0027397260273972603],
                                    'Frequency_day_weighting': [0.5, 0.5, 0, 0],
                                    'Price_file': None,
                                    },
                  'HESS_Params': {'HESS_score': 1,
                                  'Kappa': 0.25,
                                  'Zeta': 1,
                                  'BudgetLimit': 10000,
                                  'FixedCost': 0,
                                  'Int_rate': 0.05,
                                  'Lifetime': 10},
                  'Storage_Units': [{'Lithium_Battery': {'Actual_Capacity': 40,
                                                         ...
                                                         'Minimum_SOC': 10,
                                                         'Simulated': True,
                                                         'Implementation': 'simulators.NMCBatteryFirstLife',
                                                         'PowerCapCost': 5000,
                                                         'EnergyCapCost': 4000,
                                                         'Unit_Specific_Parameters': {'L_total': 0},
                                                         'Deg_cost_per_kWh': 1000}},
                                    {'Super_Capacitor': {'Actual_Capacity': 0.5,
                                                         ...
                                                         'Minimum_SOC': 10,
                                                         'Simulated': True,
                                                         'Implementation': 'simulators.SuperCapacitor',
                                                         'PowerCapCost': 5000,
                                                         'EnergyCapCost': 4000}},
                                    {'Vanadium_Battery': {'Actual_Capacity': 150,
                                                          'Available_Capacity': 1,
                                                          'Min_Limit_Energy_Capacity': 0.0001,
                                                          'Max_Limit_Energy_Capacity': 150,
                                                          'Actual_Power': 48,
                                                          'Available_Power': 1,
                                                          'Min_Limit_Power': 0,
                                                          'Max_Limit_Power': 48,
                                                          ...
                                                          'Minimum_SOC': 10,
                                                          'Simulated': True,
                                                          'Implementation': 'simulators.VanadiumBattery',
                                                          'PowerCapCost': 5000,
                                                          'EnergyCapCost': 4000}}]}
...
def initialize_config(default_config, user_config):
    ...
    for config_key, config_value in user_config.items():
        ...
        elif config_key == "Storage_Units":
            for storage_dict in config_value:
                unit_name = list(storage_dict.keys())[0]
                unit_idx = None
                for i, storage_dict_in_updated_config in enumerate(updated_config["Storage_Units"]):
                    if unit_name in storage_dict_in_updated_config.keys():
                        unit_idx = i
                for k2, v2 in storage_dict.items():
                    for k3, v3 in v2.items():
                        updated_config["Storage_Units"][unit_idx][unit_name][k3] = v3
        else:
            updated_config[config_key] = config_value

    # Remove all storage units which are not defined by the user
    user_defined_units = [list(storage_dict.keys())[0] for storage_dict in user_config["Storage_Units"]]
    updated_config["Storage_Units"] = [storage_dict_in_updated_config for storage_dict_in_updated_config in updated_config["Storage_Units"] if list(storage_dict_in_updated_config.keys())[0] in user_defined_units]
...
def Hourly_li_degradation_cost(res, config):
    ...
    for i, storage_dict in enumerate(config["Storage_Units"]):
        if "Lithium_Battery" in storage_dict.keys():
            li_idx = i

    cost_per_kWh = config["Storage_Units"][li_idx]["Lithium_Battery"]["Deg_cost_per_kWh"]  # €/kWh
...
def Select_Storage_Models_for_Lifetime(config, hess_lifetime):
    ...
    Storage_units = [list(unit.keys())[0] for unit in config.get("Storage_Units")]

    if hess_lifetime == 0:
        for idx, unit in enumerate(Storage_units):
            if unit == "Lithium_Battery":  # Currently only degradation for Lithium storage assumed
                config["Storage_Units"][idx]["Lithium_Battery"]["Implementation"] = "simulators.NMCBatteryFirstLife"
                config["Storage_Units"][idx]["Lithium_Battery"]["Unit_Specific_Parameters"]["L_total"] = 0
    else:
        for idx, unit in enumerate(Storage_units):
            if unit == "Lithium_Battery":  # Currently only degradation for Lithium storage assumed
                config["Storage_Units"][idx]["Lithium_Battery"]["Implementation"] = "simulators.NMCBatterySecondLife"
                config["Storage_Units"][idx]["Lithium_Battery"]["Unit_Specific_Parameters"][
...
def Parameterization(config, standalone=False):
    ...
    pRt_range = np.linspace(0.1, sum([d[list(d.keys())[0]]["Available_Power"] for d in config['Storage_Units']]), granularity)
    ...
    # Degradation cost
    if "Lithium_Battery" in Storage_units:
    ...
    for idx, unit in enumerate(Storage_units):
        config["Storage_Units"][idx][unit]["Loss_coef_a"] = loss_coefficients[unit][0]
        config["Storage_Units"][idx][unit]["Loss_coef_b"] = loss_coefficients[unit][1]

        if unit == "Lithium_Battery":
            config["Storage_Units"][idx]["Lithium_Battery"]["Deg_cost_coef_a"] = deg_cost_coefficients[0]
            config["Storage_Units"][idx]["Lithium_Battery"]["Deg_cost_coef_b"] = deg_cost_coefficients[1]
            config["Storage_Units"][idx]["Lithium_Battery"]["Deg_cost_coef_c"] = deg_cost_coefficients[2]
...
def Calculate_total_gradient(netOpCost_cur, netOpCost_pert, config):
    c_cap = np.array([float(d[list(d.keys())[0]][k]) for k in ["PowerCapCost", "EnergyCapCost"] for d in
                      config['Storage_Units']])  ### Get power, energy capacity and fixed cost from the config ###
...
def Calculate_suggested_capacities_from_gradient(x_cur, grad_x, config):
    ...
    scaling = np.array(
        [d[list(d.keys())[0]][k] for k in ["Available_Power", "Available_Capacity"] for d in config['Storage_Units']])

    x_min = np.array([d[list(d.keys())[0]][k] for k in ["Min_Limit_Power", "Min_Limit_Energy_Capacity"] for d in
                      config['Storage_Units']])
...
def Update_capacities(x, config):
    for j in range(int(len(x) / 2)):
        config['Storage_Units'][j][list(config['Storage_Units'][j].keys())[0]]["Available_Power"] = x[j]
        config['Storage_Units'][j][list(config['Storage_Units'][j].keys())[0]]["Available_Capacity"] = x[j + 3]

    cur_cap_info = {unit_name: [unit_info['Available_Power'], unit_info['Available_Capacity']] for d in
                    config['Storage_Units'] for unit_name, unit_info in d.items()}

    return config, x.copy(), cur_cap_info


def Store_data(x_cur, netopcost_cur, i, init_time, config):
    Storage_units = [list(d.keys())[0] for d in config["Storage_Units"]]
    c_cap = np.array([float(d[list(d.keys())[0]][k]) for k in ["PowerCapCost", "EnergyCapCost"] for d in
                      config['Storage_Units']])  ### Get power, energy capacity and fixed cost from the config ###
    FixedCost = float(config["HESS_Params"]["FixedCost"])  # Fixed cost of HESS
    int_rate, lifetime = float(config["HESS_Params"]["Int_rate"]), float(config["HESS_Params"]["Lifetime"])
    invAnnuFactor = int_rate * ((int_rate + 1) ** lifetime) / (
                (int_rate + 1) ** lifetime - 1)  # Annualized Investment Cost
    Iter_dict = {[f"{a}_{aliases[b]}" for a in ["P", "E"] for b in Storage_units][index]: [x_cur[index]] for index in
                 range(len(x_cur))}
...
def Load_params_from_config(config):
    x_cur = np.array([d[list(d.keys())[0]][k] for k in ["Available_Power", "Available_Capacity"] for d in config['Storage_Units']])
    ...
    cur_cap_info = {unit_name: [unit_info['Available_Power'], unit_info['Available_Capacity']] for d in config['Storage_Units'] for unit_name, unit_info in d.items()}
...
def Run_Daily_Schedule_Optimization(config, day=0, manual_prices=None):
    ...
    # ESS_A
    P_A = float(
        config['Storage_Units'][0][list(config['Storage_Units'][0].keys())[0]]["Available_Power"])  # Power Capacity
    E_A = float(
        config['Storage_Units'][0][list(config['Storage_Units'][0].keys())[0]]["Available_Capacity"])  # Energy Capacity
    try:
        E_A = E_A * (1 -
                     config["Storage_Units"][0][list(config['Storage_Units'][0].keys())[0]]["Unit_Specific_Parameters"][
                         "L_total"])
    except KeyError:
        pass
    Loss_coef_a_A = float(config['Storage_Units'][0][list(config['Storage_Units'][0].keys())[0]][
                              "Loss_coef_a"])  # Losses due to regulation provision
    Loss_coef_b_A = float(config['Storage_Units'][0][list(config['Storage_Units'][0].keys())[0]]["Loss_coef_b"])
    eta_ch_A = float(
        config['Storage_Units'][0][list(config['Storage_Units'][0].keys())[0]]["Eta_ch"])  # efficiency loss (charging)
    eta_dis_A = float(config['Storage_Units'][0][list(config['Storage_Units'][0].keys())[0]][
                          "Eta_dis"])  # efficiency loss (discharging)
    static_A = float(config['Storage_Units'][0][list(config['Storage_Units'][0].keys())[0]]["Static"])  # static loss
    aEmin_A = float(config['Storage_Units'][0][list(config['Storage_Units'][0].keys())[0]][
                        "Minimum_SOC"]) / 100  # minimum State of Energy (0.2 = 20%)
    aEmax_A = float(config['Storage_Units'][0][list(config['Storage_Units'][0].keys())[0]][
                        "Maximum_SOC"]) / 100  # maximum State of Energy (0.9 = 90%)
    gamma_A = float(config['Storage_Units'][0][list(config['Storage_Units'][0].keys())[0]][
                        "Gamma"])  # min{E_i/P_i, k}/k, where k = 0.25 (15 min duration), gamma = 1
    try:
        Deg_cost_coef_a_A = float(
            config['Storage_Units'][0][list(config['Storage_Units'][0].keys())[0]]["Deg_cost_coef_a"])
        Deg_cost_coef_b_A = float(
            config['Storage_Units'][0][list(config['Storage_Units'][0].keys())[0]]["Deg_cost_coef_b"])
        Deg_cost_coef_c_A = float(
            config['Storage_Units'][0][list(config['Storage_Units'][0].keys())[0]]["Deg_cost_coef_c"])
        ...
        # ESS_B
    P_B = float(
        config['Storage_Units'][1][list(config['Storage_Units'][1].keys())[0]]["Available_Power"])  # Power Capacity
    E_B = float(
        config['Storage_Units'][1][list(config['Storage_Units'][1].keys())[0]]["Available_Capacity"])  # Energy Capacity
    try:
        E_B = E_B * (1 -
                     config["Storage_Units"][1][list(config['Storage_Units'][1].keys())[0]]["Unit_Specific_Parameters"][
                         "L_total"])
    except KeyError:
        pass
    Loss_coef_a_B = float(config['Storage_Units'][1][list(config['Storage_Units'][1].keys())[0]][
                              "Loss_coef_a"])  # Losses due to regulation provision
    Loss_coef_b_B = float(config['Storage_Units'][1][list(config['Storage_Units'][1].keys())[0]]["Loss_coef_b"])
    eta_ch_B = float(
        config['Storage_Units'][1][list(config['Storage_Units'][1].keys())[0]]["Eta_ch"])  # efficiency loss (charging)
    eta_dis_B = float(config['Storage_Units'][1][list(config['Storage_Units'][1].keys())[0]][
                          "Eta_dis"])  # efficiency loss (discharging)
    static_B = float(config['Storage_Units'][1][list(config['Storage_Units'][1].keys())[0]]["Static"])
    aEmin_B = float(config['Storage_Units'][1][list(config['Storage_Units'][1].keys())[0]][
                        "Minimum_SOC"]) / 100  # minimum State of Energy (0.2 = 20%)
    aEmax_B = float(config['Storage_Units'][1][list(config['Storage_Units'][1].keys())[0]][
                        "Maximum_SOC"]) / 100  # maximum State of Energy (0.9 = 90%)
    gamma_B = float(config['Storage_Units'][1][list(config['Storage_Units'][1].keys())[0]][
                        "Gamma"])  # min{E_i/P_i, k}/k, where k = 0.25 (15 min duration) gamma = 0.1/0.25
    try:
        Deg_cost_coef_a_B = float(
            config['Storage_Units'][1][list(config['Storage_Units'][1].keys())[0]]["Deg_cost_coef_a"])
        Deg_cost_coef_b_B = float(
            config['Storage_Units'][1][list(config['Storage_Units'][1].keys())[0]]["Deg_cost_coef_b"])
        Deg_cost_coef_c_B = float(
            config['Storage_Units'][1][list(config['Storage_Units'][1].keys())[0]]["Deg_cost_coef_c"])
    ...
    # ESS_C
    P_C = float(
        config['Storage_Units'][2][list(config['Storage_Units'][2].keys())[0]]["Available_Power"])  # Power Capacity
    E_C = float(
        config['Storage_Units'][2][list(config['Storage_Units'][2].keys())[0]]["Available_Capacity"])  # Energy Capacity
    try:
        E_C = E_C * (1 -
                     config["Storage_Units"][2][list(config['Storage_Units'][2].keys())[0]]["Unit_Specific_Parameters"][
                         "L_total"])
    except KeyError:
        pass
    Loss_coef_a_C = float(config['Storage_Units'][2][list(config['Storage_Units'][2].keys())[0]][
                              "Loss_coef_a"])  # Losses due to regulation provision
    Loss_coef_b_C = float(config['Storage_Units'][2][list(config['Storage_Units'][2].keys())[0]]["Loss_coef_b"])
    eta_ch_C = float(
        config['Storage_Units'][2][list(config['Storage_Units'][2].keys())[0]]["Eta_ch"])  # efficiency loss (charging)
    eta_dis_C = float(config['Storage_Units'][2][list(config['Storage_Units'][2].keys())[0]][
                          "Eta_dis"])  # efficiency loss (discharging)
    static_C = float(config['Storage_Units'][2][list(config['Storage_Units'][2].keys())[0]]["Static"])
    aEmin_C = float(config['Storage_Units'][2][list(config['Storage_Units'][2].keys())[0]][
                        "Minimum_SOC"]) / 100  # minimum State of Energy (0.2 = 20%)
    aEmax_C = float(config['Storage_Units'][2][list(config['Storage_Units'][2].keys())[0]][
                        "Maximum_SOC"]) / 100  # maximum State of Energy (0.9 = 90%)
    gamma_C = float(config['Storage_Units'][2][list(config['Storage_Units'][2].keys())[0]][
                        "Gamma"])  # min{E_i/P_i, k}/k, where k = 0.25 (15 min duration) gamma = 0.1/0.25
    try:
        Deg_cost_coef_a_C = float(
            config['Storage_Units'][2][list(config['Storage_Units'][2].keys())[0]]["Deg_cost_coef_a"])
        Deg_cost_coef_b_C = float(
            config['Storage_Units'][2][list(config['Storage_Units'][2].keys())[0]]["Deg_cost_coef_b"])
        Deg_cost_coef_c_C = float(
            config['Storage_Units'][2][list(config['Storage_Units'][2].keys())[0]]["Deg_cost_coef_c"])
    ...
    model.pRt_A = Var(model.T, domain=NonNegativeReals)  # Regulation allocated
    ...
    model.pRt_B = Var(model.T, domain=NonNegativeReals)  # Regulation allocated
    ...
    model.pRt_C = Var(model.T, domain=NonNegativeReals)  # Regulation allocated
    ...
    def HESS_pEt_def_rule(model, tt):
        return model.pEt[tt] == model.pEt_A[tt] + model.pEt_B[tt] + model.pEt_C[tt]
    ...
    def ESS_pEt_A_def_rule(model, tt):
        return model.pEt_A[tt] == model.pBt_A[tt] * dt + Loss_coef_a_A * model.pRt[tt] + Loss_coef_b_A
